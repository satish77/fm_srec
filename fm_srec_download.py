...
class SRecord:
    def __init__(self, init=0xff, checkcs=True):
        ...
        self.init   = init
        self.check  = checkcs
        self.x1 = win32com.client.dynamic.Dispatch("MCB.PCM")

    def ReadVariable (self, var):
        val1, val2, val3, val4  = self.x1.ReadVariable(var)
        if val1==True:
            return val2
        else:
            raise Exception("Freemaster read failed!!!")
        
    def WriteVariable (self, var, val):
        val1 = self.x1.WriteVariable(var, val)
        if val1==False:
            raise Exception("Freemaster write failed!!!")
    def WriteArray(self, var, val, size):
        """ caution: 50 bytes limitation in Freescale driver found during testing """
        #val1 = self.x1.WriteVariable(var, val, size)
        val1 = self.x1.WriteUIntArray(var, size, 1, val)
        if val1==False:
            raise Exception("Freemaster write failed!!!")      
...
##            return
        ...
        fprgCSW_val = self.ReadVariable("fprgCSW")
        while((fprgCSW_val != 0) and (fprgCSW_val != 1)):
            fprgCSW_val = self.ReadVariable("fprgCSW")
            time.sleep(100)
            pass
        if fprgCSW_val == 1:
